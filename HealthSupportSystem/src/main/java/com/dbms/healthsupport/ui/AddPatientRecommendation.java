...
public class AddPatientRecommendation extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AddPatientRecommendation frame = new AddPatientRecommendation("P1");
...
//Integer recId, String frequencyName, Integer threshold, String observationSpecification
	 ...
	 * Create the frame.
	 */
	public AddPatientRecommendation(String currPatientSSN) {
		...
		JLabel lblAddRecommendation = new JLabel("Add Patient Recommendation for: " + currPatientSSN);
		lblAddRecommendation.setBounds(18, 20, 300, 16);
		...
		frequencies = new FrequencyDao().getAllData();
			...
			JOptionPane.showMessageDialog(AddPatientRecommendation.this,
				    e.getMessage(),
				    "Inane warning",
		...
		btnAddRecommendation.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try
				{
					Recommendation recommendation = new RecommendationDao().insertPatientRecommendation(new Recommendation(0, textField_1.getSelectedItem().toString(), Integer.parseInt(textField_2.getText()), textField_3.getSelectedItem().toString()), new PatientDao().getDataById(patientSSN));
					JOptionPane.showMessageDialog(AddPatientRecommendation.this,
