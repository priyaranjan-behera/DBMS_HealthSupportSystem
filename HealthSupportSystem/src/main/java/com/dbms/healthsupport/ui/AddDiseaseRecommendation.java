...
public class AddDiseaseRecommendation extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AddDiseaseRecommendation frame = new AddDiseaseRecommendation("HIV");
...
//Integer recId, String frequencyName, Integer threshold, String observationSpecification
	 ...
	 * Create the frame.
	 */
	public AddDiseaseRecommendation(String currDiseaseName) {
		...
		JLabel lblAddRecommendation = new JLabel("Add Disease Specific Recommendation for :" + diseaseName);
		lblAddRecommendation.setBounds(18, 20, 250, 16);
		...
		frequencies = new FrequencyDao().getAllData();
			...
			JOptionPane.showMessageDialog(AddDiseaseRecommendation.this,
				    e.getMessage(),
				    "Inane warning",
		...
		btnAddRecommendation.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try
				{
					Recommendation recommendation = new RecommendationDao().insertDiseaseRecommendation(new Recommendation(0, textField_1.getSelectedItem().toString(), Integer.parseInt(textField_2.getText()), textField_3.getSelectedItem().toString()), new DiseasesDao().getDataById(diseaseName));
					JOptionPane.showMessageDialog(AddDiseaseRecommendation.this,
